...
parser = argparse.ArgumentParser(
    ...
    ^1University of Montana, Bioscience
...
parser.add_argument('-mode',
                    help="paired (if you are providing a directory with paired-end reads) or single (if your folder"
...
os.system("cat %s/*%s > combined_reads.fastq" % (args.reads_directory, args.reads_ext))
...
os.system("bowtie2-build --threads " + str(args.num_threads) + " -f " + args.bin_directory + "/combined_contigs.fasta "
          + args.bin_directory + "/combined_contigs.fasta")
...
if args.bin_comparisons == "NA":
    out.write(
        "bin" + "," + "read_proportion_when_mapped_together" + "," + "read_proportion_when_mapped_separately" + "\n")
else:
    out.write(
        "bin" + "," + "read_proportion_when_mapped_together" + "," + "read_proportion_when_mapped_separately" + ",")
    ...
    if args.bin_comparisons == "NA":
        ...
        for j in CovDictComp.keys():
            if len(CovDictComp[j][bin]) == 0:
                ...
                print(bin + "recruited " + str(totalReadsComp) + " reads during bin_comparison_" + str(
                    j.split(".")[0]) + "\n")
    out.write("\n")
    print("")
